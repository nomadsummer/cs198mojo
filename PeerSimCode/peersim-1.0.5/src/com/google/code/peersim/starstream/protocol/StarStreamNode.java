 ...
 * This class represents a node that leverages both the *-Stream and the Pastry
 * protocols to take part to a P2P streaming event.<br>
 * Extending {@link PastryNode} this class can take advantage of all the
 * features and control classes available for {@link PastryNode}.
 * 
 * @author frusso
 * @version 0.1
 * @since 0.1
 */
public class StarStreamNode extends PastryNode implements
		StarStreamProtocolListenerIfc {

	/**
	 * Configuration parameter key that ties a *-Stream node to its *-Stream
	 * protocol.
	 */
	public static final String STAR_STREAM = "starstream";
	/**
	 * The protocol identifier assigned to the *-Stream protocol by the PeerSim
	 * runtime.
	 */
	...
	// useful to know which chunks were requested for but not discovered, at the
	 ...
	 * 
	 * @param prefix
	 *            The prefix for accessing configuration properties
	 * @throws java.io.FileNotFoundException
	 *             Raised whenever it is not possible creating the log file
	 */
	public StarStreamNode(String prefix) throws FileNotFoundException {
		...
		// takeing
		// references to Pastry and *-Stream and tight the two instance enabling
		// the
		...
		START_STREAMING_TIMEOUT = (int) Math.ceil(Configuration
				.getDouble(prefix + SEPARATOR + "startStreamingTimeout"));
		WAIT_BETWEEN_FORCES = Configuration.getInt(prefix + SEPARATOR
				+ "waitBetweenForces");
	...
	public void notifyNewChunk(Chunk<?> chunk) {
		log("[*-STREAM] node " + this.getPastryId() + " has stored resource "
				+ chunk);
	...
	/**
	 * Tells the associated {@link StarStreamProtocol} instance that there has
	 * been a new simulated-time tick and that both the outbound and inbound
	 * bandwiths can be reset to their original levels.
	 */
	...
	public void getBandwidth() {
		...
		System.err.println("Current Time: " + CommonState.getTime()
				+ " | UpStream: " + thisup);
		System.err.println("Current Time: " + CommonState.getTime()
				+ " | DownStream: " + thisdown);
	}
	...
	public void tick() {
		...
		if (isJoined() && streamingStartTime != null) {
			...
			// writer.println("[MOJO] "+ CommonState.getTime()+" "+ this.getID()
			// + " DL: "+ this.getPlayedChunks().size()*16);

			// if(this.getPlayedChunks().size() > 0 &&
			// this.getPlayedChunks().size()+1 < 101 && played !=
			// this.getPlayedChunks().size())
			// writer.println("[MOJO] "+ CommonState.getTime()+" "+ this.getID()
			// + " PlayedChunks: "+ this.getPlayedChunks().size());
			// writer.println("[MOJO] "+
			// this.perceivedChunkDeliveryTimes.getAverage());
			played = this.getPlayedChunks().size();

			// if(this.getUnplayedChunks().size() > 0 &&
			// this.getUnplayedChunks().size()+1 < 101 && unplayed !=
			// this.getUnplayedChunks().size())
			// writer.println("[MOJO] "+ CommonState.getTime()+" "+ this.getID()
			// + " UnPlayedChunks: "+ this.getUnplayedChunks().size() + " " +
			// this.getUnplayedChunks());
	...
	/**
	 * Returns a reference to this node's assigned {@link StarStreamProtocol}
	 * instance.
	 * 
	...
	/**
	 * Tells the *-Stream protocol to check for expired messages that were
	 * waiting for acks/nacks and behave consequently.
	 */
	...
	private void checkForStartStreamingTimeout() {
		if (!player.isStarted()
				&& CommonState.getTime() >= (streamingStartTime + START_STREAMING_TIMEOUT)) {
			if (CommonState.getTime() > lastForce + WAIT_BETWEEN_FORCES) {
				// driiin!!! timeout expired
				// start proactive search (pull) for those chunks required to
				// fill in
				// the buffer
	...
	private void issueChunkRequest(UUID sessionId, int nextChunkSeqId) {
		PastryId chunkId = ChunkUtils.getChunkIdForSequenceId(sessionId,
				nextChunkSeqId);
		if (chunkId != null) {
			log("[*-STREAM] node " + this.getPastryId()
					+ " starts looking for chunk(" + nextChunkSeqId + ") "
					+ chunkId);
			/*
			 * if (isHelping()) { if (Math.random() < .20) {
			...
			log("[*-STREAM] WARN node " + this.getPastryId()
					+ " No one knows anything about chunk " + nextChunkSeqId);
		}
	}
	...
	private SortedSet<Integer> getMissingChunkSeqIdsLessThan(int seqId) {
		...
		if (first < seqId) {
			for (int i = seqId - 1; i > first; i--) {
				if (!deliveredChunks.contains(i)
						&& !chunkRequestsForSeqIdsWithoutPastryIdYet
								.contains(i)) {
					missings.add(i);
				}
			}
	...
	private void proactiveSearch() {
		if (player.isStarted()
				&& (CommonState.getTime() > streamingStartTime
	...
	private void processDelayedChunkRequests() {
		UUID sessionId = StarStreamSource.getStarStreamSessionId();
		Integer[] pcrs = chunkRequestsForSeqIdsWithoutPastryIdYet
				.toArray(new Integer[chunkRequestsForSeqIdsWithoutPastryIdYet
						.size()]);
	...
	private void scheduleNextChunkRequest() {
		...
		double avgObservedDeliveryTime = (perceivedChunkDeliveryTimes.getN() == 0) ? 0
				: perceivedChunkDeliveryTimes.getAverage();
		double num = currentTime + avgObservedDeliveryTime
				- player.getWhenPlaybackStarted() + advance;
		int nextChunkSeqId = Double.valueOf(
				Math.floor(num / chunkPlaybackLength)).intValue();
		if (StarStreamSource.isSeqIdLegal(nextChunkSeqId)
				&& !hasBeenDelivered(nextChunkSeqId)) {
			// once the next seq-id has been computed we have to:
			// 1. store somewhere that the i-th chunk has been scheduled for
			// search right now
			// 2. start searching for that chunk iff this is the very first time
			// we scheduled it
			// NOTE: pro-actively searching for a chunk entails two steps:
			// 1. ask some neighbors if anyone of them actually has the chunk
			// 2. either in the event of nacks or in case of req-timeout
			// expiration, issue
			// a Pastry lookup for that chunk: the Pastry lookup already has its
			// own
			// configurable retries
			// Thus, according to the two observations above, it is not
	...
	private void checkIfPlaybackIsAllowed() {
		int contiguousChunks = getStore().countContiguousChunksFromStart(
				StarStreamSource.getStarStreamSessionId());
	...
	private List<Integer> collectMissingChunkIds() {
		List<Integer> seqIds = getStore().getMissingSequenceIds(
				StarStreamSource.getStarStreamSessionId());
		if (seqIds.size() == 0) {
			if (aggressive) {
				// we are really unlucky, the buffer is completely empty, but
				// luckly we know
				// we can search for chunks with seqIDs from 0 to whatever we
	 ...
	 * let the {@link StarStreamProtocol} instance register over the
	 * {@link PastryProtocol} instance for Pastry-related events.
	 */
	private void init() {
		...
		this.lastForce = 0;
		this.player = new StarStreamPlayer(this,
				StarStreamSource.getStarStreamSessionId(), chunkPlaybackLength,
				totalChunks);
	...
	private void updateLocalStats(Chunk<?> chunk) {
		...
		// [MOJO]
		// System.err.println(chunk.getSequenceId()+1);
		// perceivedChunkDeliveryTimes.add(deliveryTime/(chunk.getSequenceId()+1));
