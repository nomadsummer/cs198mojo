...
public class StarStreamProtocol implements EDProtocol,
	...
	public StarStreamProtocol(String prefix) throws FileNotFoundException {
		...
		// CommonState.getNetworkSize();
		minDownStream = Configuration
				.getInt("protocol.mojocollab.minDownStreamPerPeer");
		maxDownStream = Configuration
				.getInt("protocol.mojocollab.maxDownStreamPerPeer");
		minUpStream = Configuration
	 ...
	 * 
	 * //[MOJO] public int getTimeStay(){ return timeStay; }
	 */
	...
	void resetUsedBandwidth() {
		// System.out.println(this.owner.getID() +
		// " Remaining Bandwidth before resetting... up: " + usedUpStream +
		// " down: " + usedDownStream);
		...
		if ((int) this.owner.getID() < CommonState.getNetworkSize()) {
			...
			CommonState.bandwidthUtilUp[(int) this.owner.getID()]
					.add(bandWidthUtil);
			remainingBandwidth = getDownStream() - usedDownStream;
			bandWidthUtil = 1 - (remainingBandwidth) / getDownStream();
			CommonState.bandwidthUtilDown[(int) this.owner.getID()]
					.add(bandWidthUtil);
		}
		usedDownStream = 0;
		usedUpStream = 0;
	...
	private boolean updateUsedDownStream(StarStreamMessage msg) {
		...
		if (usedDownStream + msg.getType().getEstimatedBandwidth() <= getDownStream()) {
			...
			proceed = true;
			// System.out.println(this.owner.getID() +
			// " chunk message required bandwidth: " +
			// msg.getType().getEstimatedBandwidth());
