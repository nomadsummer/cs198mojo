...
=======
public class StarStreamProtocol implements EDProtocol, CDProtocol,
		PastryProtocolListenerIfc {
	...
	private boolean corruptedMessages;
	/**
	 * Whether messages should be corruptable or not. Legal values are in
	 * [0..1].
	 */
	public static final String CORRUPTED_MESSAGES_PROB = "corruptedMessagesProbability";
	/**
	...
	private float corruptedMessagesProbability;
	/**
	 * How many simulation-time units a node should try and send a chunk to
	 * another one.
	 */
	public static final String MAX_CHUNK_RETRIES = "maxChunkRetries";
	/**
	...
	public static final String REL_TRANSPORT = "reliableTransport";
	/**
	 * The protocol id assigned by the PeerSim runtime to the reliable transport
	 * instance.
	 */
	private int reliableTransportPid;
	/**
	...
	public static final String LOG_FILE = "log";
	/**
	 * Property for configuring whether the protocol should log its activity or
	 * not.
	 */
	public static final String DO_LOG = "doLog";
	/**
	...
	private List<StarStreamProtocolListenerIfc> listeners = new ArrayList<StarStreamProtocolListenerIfc>();
	/**
	 * Memory that associates a *-Stream message identifier with the message it
	 * refers to. The identifier is meant to be matched to incoming messages'
	...
	public StarStreamProtocol(String prefix) throws FileNotFoundException {
		msgTimeout = Configuration.getInt(prefix + SEPARATOR + MSG_TIMEOUT);
		starStoreSize = Configuration.getInt(prefix + SEPARATOR
				+ STAR_STORE_SIZE);
		reliableTransportPid = Configuration.getPid(prefix + SEPARATOR
				+ REL_TRANSPORT);
		doLog = Configuration.getBoolean(prefix + SEPARATOR + DO_LOG);
		if (doLog) {
			stream = new PrintStream(new FileOutputStream(
					new FileNameGenerator(Configuration.getString(prefix
							+ SEPARATOR + LOG_FILE), ".log").nextCounterName()));
		}
		corruptedMessages = Configuration.getBoolean(prefix + SEPARATOR
				+ CORRUPTED_MESSAGES);
		if (corruptedMessages) {
			corruptedMessagesProbability = (float) Configuration
					.getDouble(prefix + SEPARATOR + CORRUPTED_MESSAGES_PROB);
		}
		downStream = Configuration.getInt(prefix + SEPARATOR + "downStream")
				* CommonState.getNetworkSize();
		...
		timeIn = Configuration.getInt("protocol.mojocollab.timeIn");
		timeStay = Configuration.getInt("protocol.mojocollab.timeStay");
		aggressionFactor = Configuration
				.getDouble("protocol.mojocollab.aggressionFactor");
		eqnReliance = Configuration
				.getDouble("protocol.mojocollab.eqnReliance");
		defaultResourceFactor = Configuration
				.getDouble("protocol.mojocollab.defaultResourceFactor");
		System.err.println("Down: " + downStream + " | Up: " + upStream);
		System.err.println("DownPerPeer: " + downStreamPerPeer
				+ " | UpPerPeer: " + upStreamPerPeer);
		System.err.println("AggressionFactor: " + aggressionFactor
				+ " | EquationReliance: " + eqnReliance);
		System.err.println("DefaultResourceFactor: " + defaultResourceFactor
				+ " | HelpingPeers: " + CommonState.getHelping());
		System.err.println("TimeIn: " + timeIn + " | TimeStay: " + timeStay);
		// [MOJO]

		maxChunkRetries = Configuration.getInt(prefix + SEPARATOR
				+ MAX_CHUNK_RETRIES);
		store = new StarStreamStore(starStoreSize);
		aggressive = Configuration
				.getBoolean(prefix + SEPARATOR + "aggressive");
	}
	...
	public final Object clone() {
		try {
			...
			throw new RuntimeException(
					"Cloning failed. See nested exceptions, please.", e);
		}
	}
	...
	/*public void printBandwidth(){
		System.err.println("UP: "+ getUpStream());
		System.err.println("DOWN: "+ getDownStream());
	}*/
	...
	// [MOJO]
	/*
	 * public int numHelpingPeers(){ return helpingPeers; }
	 */
	...
	/**
	 * Returns the number of chunks received by means of the Pastry API.
	 * 
	 * @return The number of chunks received by means of the Pastry API.
	 */
	public int getChunksReceivedFromPastry() {
		return chunksReceivedFromPastry;
	}
	...
	/**
	 * Returns the number of chunks received by means of the StarStream API.
	 * 
	 * @return The number of chunks received by means of the StarStream API.
	 */
	public int getChunksReceivedFromStarStream() {
		return chunksReceivedFromStarStream;
	}
	...
	/**
	 * Returns the number of unsent chunks due to message timeout.
	 * 
	 * @return The number of unsent chunks due to message timeout.
	 */
	public int getUnsentChunkMsgsDueToTimeout() {
		return unsentChunkMsgsDueToTimeout;
	}
	...
	/**
	 * Routes the event, that must be assignable to {@link StarStreamMessage},
	...
	@Override
	public void processEvent(Node localNode, int thisProtocolId, Object event) {
		// System.out.println("[MOJO] " + CommonState.getTime() + " id: " +
		// localNode.getID() + " upload: " + this.usedUpStream);
		// System.out.println("[MOJO] " + CommonState.getTime() + " id: " +
		// localNode.getID() + " download: " + this.usedDownStream);
		// event-handling logic begins
		if (event instanceof StarStreamMessage) {
			// this is a known event, let's process it
			StarStreamMessage msg = (StarStreamMessage) event;
			...
			if (updateUsedDownStream(msg)) {
				...
				// there is not enough input-stream to consume the message: the
				// message must
				// be cached for later use
				addToDelayedInMessages(msg);
			}
			...
			// an unknown event has been received
			throw new IllegalStateException("An event of type "
					+ event.getClass()
					+ " has been received, but I do not know how to handle it.");
		}
	}
	 ...
	 */
	void processDelayedMessages() {
		StarStreamMessage[] inMsgs = new StarStreamMessage[delayedInMessages
				.size()];
		delayedInMessages.<StarStreamMessage> toArray(inMsgs);
		StarStreamMessage[] outMsgs = new StarStreamMessage[delayedOutMessages
				.size()];
		delayedOutMessages.<StarStreamMessage> toArray(outMsgs);
	...
	/**
	 * Adds the given {@link StarStreamMessage} to the set of delayed input
	...
	/**
	 * Adds the given {@link StarStreamMessage} to the set of delayed output
	 * messages for later processing.
	 * 
	 * @param msg
	 *            The message
	...
	/**
	 * Internal logic for processing incoming messages only if they have been
	 * already accepted.
	 * 
	 * @param msg
	...
	private void processEvent(StarStreamMessage msg) {
		...
		default: {
			throw new IllegalStateException("A message of type "
					+ msg.getType()
					+ " has been received, but I do not know how to handle it.");
		}
		}
	 ...
	 * <li>advertise that resource to a set of randomly choosen neighbors</li>
	 * </ol>
	 * <b>Note:</b> no further routing is required since the
	 * {@link PastryProtocol} instance is charge of that.
	 * 
	 * {@inheritDoc}
	 */
	@Override
	...
	/**
	 * This method must be invoked to check whether there are expired
	 * {@link ChunkMessage}s}. Each expired message gets removed from the memory
	 * and is sent again iff the message has not been sent for the maximum
	 * amount of times yet.
	 */
	void checkForTimeouts() {
		long currentTime = CommonState.getTime();
	...
	private void checkForTimeoutsInPendingChunkMessages(long currentTime) {
		List<UUID> expired = new ArrayList<UUID>();
		// search
		for (Map.Entry<UUID, ChunkMessage> pending : pendingChunkMessages
				.entrySet()) {
			if (pending.getValue().getTimeStamp() + msgTimeout < currentTime) {
				// this message has expired!
				expired.add(pending.getKey());
		...
		for (UUID id : expired) {
			...
			if (msg != null) {
				log("[TIMEOUT] (+"
						+ (currentTime - (msgTimeout + msg.getTimeStamp()))
						+ ") " + msg);
				// resend iff the retry-time has not reached the configured max
				// amount yet
				if (msg.getRetries() < maxChunkRetries) {
					// resend
					msg.prepareForRetry();
					...
					log("[NOT SENT] " + MAX_CHUNK_RETRIES + " "
							+ maxChunkRetries + " reached");
					unsentChunkMsgsDueToTimeout++;
				}
			}
		}
	...
	private void checkForTimeoutsInPendingChunkRequests(long currentTime) {
		List<UUID> expired = new ArrayList<UUID>();
		// search
		for (Map.Entry<UUID, ChunkRequest> pending : pendingChunkRequests
				.entrySet()) {
			if (pending.getValue().getTimeStamp() + msgTimeout < currentTime) {
				// this message has expired!
				expired.add(pending.getKey());
		...
		for (UUID id : expired) {
			...
			if (msg != null) {
				log("[TIMEOUT] (+"
						+ (currentTime - (msgTimeout + msg.getTimeStamp()))
						+ ") " + msg);
				// NOTE: there is no need to check whether the chuunk has
				// already been
				// received since this is done by the method we are going to
				// invoke
				owner.lookupResource(msg.getChunkId());
			}
		}
	...
	/**
	 * This method has to be invoked only once by the {@link StarStreamNode}
	 * instance that owns this {@link StarStreamProtocol} instance to let the
	 * latter register itself as a listener for Pastry protocol-events.
	 * 
	 * @param pastry
	 *            The {@link PastryProtocol} to register on for event
	 *            notifications
	 */
	void registerPastryListeners(PastryProtocol pastry) {
		pastryProtocol = pastry;
	...
	/**
	 * Tells the {@link StarStreamProtocol} instance that there has been a new
	 * simulated-time tick and that both the outbound and inbound bandwiths can
	 * be reset to their original levels.
	 */
	void resetUsedBandwidth() {
		usedDownStream = 0;
		usedUpStream = 0;
	 ...
	 * Starts searching for the specified chunk.
	 * 
	 * @param starStreamSessionId
	 *            The *-Stream session ID
	 * @param chunkId
	 *            The chunk ID
	 */
	void searchForChunk(UUID starStreamSessionId, PastryId chunkId) {
		// Set<StarStreamNode> nodes =
		// this.owner.getPastryProtocol().getNeighbors(1);
		Set<StarStreamNode> nodes = this.owner.getPastryProtocol()
				.getNeighbors(this.availableOutDeg(Type.CHUNK_REQ));
		if (!nodes.isEmpty()) {
			StarStreamNode dst = nodes.iterator().next();
			ChunkRequest req = new ChunkRequest(owner, dst,
					starStreamSessionId, chunkId);
			if (send(req)) {
				// cache for on-timeout expiration retries
				rememberPendingChunkRequest(req);
			} else {
				// the message could not be sent due to bandwidth
				// unavailabilty
				// it will be sent as soon as possible, thus NOP
			}
		} else {
	...
	/**
	 * Advertises the existence of a new chunk to a selection of neighbors. The
	 * number of neighbors is based on how many ougoing connections this node is
	 * able to create.<br>
	 * <b>Note:</b> advertisements travel over a reliable transport<br>
	 * <b>Note:</b> as the *-Stream protocol says, this message must not be
	 * necessarly followed by anyother message: a receiving node might reply or
	 * not according to its interest in the advertised chunk
	 * 
	 * @param msg
	 *            The message containing the chunk that has to be advertised:
	 *            can be {@code null} iff {@code isOnPastryEvent} is
	 *            {@link Boolean#TRUE}
	 * @param isOnPastryEvent
	 *            Must be {@link Boolean#TRUE} iff this method is invoked since
	 *            a new {@link Chunk} has been received as a Pastry event. If
	 *            this is the case the following {@link Chunk} parameter must be
	 *            not {@code null}
	 * @param chunk
	 *            The {@link Chunk} that must be advertised: not {@code null}
	 *            iff {@code isOnPastryEvent} is {@link Boolean#TRUE}
	 */
	private void advertiseChunk(ChunkMessage msg, boolean isOnPastryEvent,
			Chunk<?> chunk) {
		Set<StarStreamNode> neighbors;
		if (!aggressive) // use max concurrent upload capacity for chunks
		{
	...
	/**
	 * Tells how many input connections can be established before exhausting the
	 * input bandwidth by receiving only messages of the given message type.
	 * 
	 * @param msgType
	 *            The type of the message(s) the protocol would like to receive
	 * @return The maximum number of connections available for the given message
	 *         type
	 */
	private int availableInDeg(Type type) {
		return (getDownStream() - usedDownStream)
				/ type.getEstimatedBandwidth();
	}

	/**
	 * Tells how many output connections can be established before exhausting
	 * the output bandwidth by sending only messages of the given message type.
	 * 
	 * @param msgType
	 *            The type of the message(s) the protocol would like to send
	 * @return The maximum number of connections available for the given message
	 *         type
	 */
	private int availableOutDeg(Type type) {
	...
	/**
	 * Sends each message stored in the provided input over the configured
	 * reliable transport.
	 * 
	 * @param msgs
	 *            The messages
	 */
	private void broadcastOverReliableTransport(
			List<? extends StarStreamMessage> msgs) {
		for (StarStreamMessage msg : msgs) {
			sendOverReliableTransport(msg);
		}
	}
	...
	/**
	 * Tells whether the message can be consumed or not. This is stated basing
	 * on the {@link StarStreamProtocol#curruptedMessagesProbability} configured
	 * value.
	 * 
	 * @param chunkMsg
	 * @return Whether the message can be consumed or not
	...
	/**
	 * When a new chunk is received, the receiving protocol instance has to do
	 * two things "concurrently":
	 * <ol>
	 * <li>immediately reply the sending node with either a {@link ChunkOk} or
	 * {@link ChunkKo} message (the integrity of the message can be verified by
	 * means of the
	 * {@link StarStreamProtocol#checkMessageIntegrity(ChunkMessage)};<br>
	 * <br>
	 * <b>NOTE:</b> in case of corrupted message, we must abort the processing<br>
	 * <br>
	 * ask itself if it already owns the chunk; should it not be the case, store
	 * the chunk locally. In either case, it must now adverties the received
	 * chunk to a set of <i>outDeg</i> neighbors. <i>outDeg</i> means the number
	 * of outgoing connections the node can currently establish. How many
	 * outgoing connections can be used at the moment is defined by the
	 * {@link StarStreamProtocol#availableOutDeg(com.google.code.peersim.starstream.protocol.StarStreamProtocol.NetworkOperation)}
	 * method</li>
	 * <li>iff the sender is the <i>source-node</i>, route the chunk over the
	 * Pastry network</li>
	 * </ol>
	 * 
	 * @param chunkMessage
	...
	private void handleChunk(ChunkMessage chunkMessage) {
		...
		if (!chunkMessage.getChunk().isExpired()) {
			...
			log("[*** EXPIRED ***] message " + chunkMessage
					+ " contains a chunk that exceeded its TTL: not delivered!");
		}
	}
	 ...
	 * receiving that chunk</li>
	 * </ol>
	 * <b>Note:</b> both the {@link ChunkAdvertisement} and the
	 * {@link ChunkRequest} travel over the reliable transport available to each
	 * {@link StarStreamProtocol} instance.
	 * 
	 * @param chunkAdvertisement
	 *            The chunk advertisement
	 */
	private void handleChunkAdvertisement(ChunkAdvertisement chunkAdvertisement) {
		log("[RCV] " + chunkAdvertisement);
		if (!store.isStored(chunkAdvertisement.getSessionId(),
				chunkAdvertisement.getChunkId())) {
			// the chunk is not locally available, thus we need to reply to the
			// advertising
			// node with a chunk request message and wait for the chunk to
			// arrive
			ChunkRequest chunkReq = chunkAdvertisement.replyWithChunkReq();
			send(chunkReq);
		} else {
			// the chunk is already stored in the local *-Stream store, thus
			// there is no
			// need and doing anything else
			// NOP
		}
	...
	/**
	 * When a {@link ChunkKo} message is received, we know that the sender of
	 * that message was not able (for whichever reason) to properly receive the
	 * chunk the message refers to.<br>
	 * For this reason, the protocol states that a node receiving a
	 * {@link ChunkKo} message must:
	 * <ol>
	 * <li>check to see whether the chunk is locally available or not</li>
	 * <li>if the chunk was actually available the node should send it using a
	 * {@link ChunkMessage}, otherwise a {@link ChunkMissing} message must be
	 * sent</li>
	 * </ol>
	 * 
	 * @param chunkKo
	...
	/**
	 * A {@link StarStreamProtocol} instance can receive a {@link ChunkMissing}
	 * message only in response to a previously issued {@link ChunkRequest}
	 * message. Obviously, a {@link ChunkMissing} message reports that a
	 * requested chunk could not be provided by the inquired node. Should this
	 * happen, the protocol prescribes that the chunk must be searched for using
	 * the underlying {@link PastryProtocol} instance, issueing a
	 * {@link PastryProtocol#lookupResource(com.google.code.peersim.pastry.protocol.PastryId, int)}
	 * .
	 * 
	 * @param chunkMissing
	 *            The message
	...
	private void handleChunkMissing(ChunkMissing chunkMissing) {
		log("[RCV] " + chunkMissing);
		// the only thing we need to do is launching a Pastry resource lookup
		// operation
		// once and if the resource is found, this protocol instance will be
		// notified by
		// the underlying Pastry implementation and the resource will be finally
		// stored
		// in the local *-Stream store
		removeFromPendingChunkRequests(chunkMissing);
		pastryProtocol.lookupResource(chunkMissing.getChunkId());
	...
	/**
	 * When a {@link ChunkOk} message is received, it means that a node we
	 * previously sent a {@link ChunkMessage} has successfully received the
	 * chunk. As far as the *-Stream protocol is concerned, at this point any
	 * other activity is foreseen for the node that received such a message.<br>
	 * <b>Note:</b> since {@link ChunkMessage}s are sent over an unreliable
	 * transport, upon reception of a {@link ChunkOk} message it is necessary
	 * purging the memory of {@link ChunkMessage}s pending acks to avoid useless
	 * retransmissions.
	 * 
	 * @param chunkOk
	 *            The OK message
	...
	/**
	 * A request for a chunk, that is a {@link ChunkRequest} message can be
	 * received:
	 * <ol>
	 * <li>either in response to a previously issued {@link ChunkAdvertisement}</li>
	 * <li>or as the effect of a proactive search initiated by a *-Stream node</li>
	 ...
	 * <ol>
	 * <li>check whether the requested chunk is locally available or not</li>
	 * <li>if the chunk is available, reply with a {@link ChunkMessage} message;
	 * <br>
	 * if the chunk is not available, reply with a {@link ChunkMissing} message</li>
	 * </ol>
	 * <b>Note:</b> the {@link ChunkMessage} has to travel over the unreliable
	 * transport while the {@link ChunkMissing} has to travel over the reliable
	 * transpor.
	 * 
	 * @param chunkRequest
	 */
	...
	/**
	 * Reply to the node that just sent the input {@link ChunkMessage} with a
	 * {@link ChunkKo} message stating that the chunk has not been properly
	 * received.<br>
	 * <b>Note:</b> this message is sent over a reliable transport.<br>
	 * <b>Note:</b> according to the protocol specification, this message could
	 * be followed by a new {@link ChunkMessage} carrying the chunk that could
	...
	/**
	 * Reply to the node that just sent the input {@link ChunkMessage} with a
	 * {@link ChunkOk} message stating that the chunk has been properly
	 * received.<br>
	 * <b>Note:</b> this message is sent over a reliable transport.<br>
	 * <b>Note:</b> the protocol specifies this message must not be followed by
	 * anyother message.<br>
	 * 
	 * @param chunkMessage
	 *            The message to reply to
	...
	/**
	 * Adds the given {@link ChunkMessage} to the set of {@link ChunkMessage}s
	 * waiting for an ack.
	 * 
	 * @param chunkMessage
	 *            The message
	...
	/**
	 * Adds the given {@link ChunkRequest} to the set of {@link ChunkRequests}s
	 * waiting for an ack.
	 * 
	 * @param chunkMessage
	 *            The message
	...
	/**
	 * Removes the {@link ChunkMessage}, identified by the correlation-id found
	 * in the input message, from the set of {@link ChunkMessage}s waiting for
	 * an ack.
	 * 
	 * @param msg
	 *            The message
	...
	/**
	 * Removes the {@link ChunkRequest}, identified by the correlation-id found
	 * in the input message, from the set of {@link ChunkRequest}s waiting for
	 * an ack.
	 * 
	 * @param msg
	 *            The message
	...
	/**
	 * Randomly selects neighbors according to Pastry's <i>neighbor</i>
	 * definition.
	 * 
	 * @param type
	 *            The type of the message(s) the protocol would like to send
	...
	/**
	 * Generic send method that must be used in place of anyother
	 * {@code sendXxx} method.
	 * 
	 * @param msg
	 *            The message
	 * @return Whether the message has been sent ({@link Boolean#TRUE}) or
	 *         delayed ({@link Boolean#FALSE})
	 */
	private boolean send(StarStreamMessage msg) {
		boolean sent = false;
		if (StarStreamMessage.Type.CHUNK.equals(msg.getType())) {
	...
	private boolean sendChunkMessage(ChunkMessage msg) {
		...
		if (!msg.getChunk().isExpired()) {
			...
			log("[*** EXPIRED ***] chunk " + msg.getChunk()
					+ " expired, chunk message not sent");
		}
		return sent;
	}

	/**
	 * Send the input {@link StarStreamMessage} over the configured reliable
	 * transport.
	 * 
	 * @param msg
	 *            The message
	...
	private boolean sendOverReliableTransport(StarStreamMessage msg) {
		...
		if (updateUsedUpStream(msg)) {
			Transport t = (Transport) owner.getProtocol(reliableTransportPid);
			t.send(msg.getSource(), msg.getDestination(), msg,
					owner.getStarStreamPid());
			log("[SND] " + msg);
			sent = true;
			sentMessages++;
		} else {
			// up-stream has been exhausted, the message must be delayed or
			// dropped
			addToDelayedOutMessages(msg);
		}
		return sent;
	...
	/**
	 * Send the input {@link StarStreamMessage} over the configured unreliable
	 * transport.
	 * 
	 * @param msg
	 *            The message
	...
	/**
	 * Adds the given chunk to the local *-Stream Store iff that chunk is not
	 * yet in the store.
	 * 
	 * @param chunk
	 *            The chunk
	...
	private boolean storeIfNotStored(Chunk<?> chunk) {
		...
		if (stored) {
			...
			// the chunk has not been added to the local store: this means it
			// was
			// already there
			// NOP
		}
	...
	/**
	 * Utility method useful upon reception of either a {@link ChunkRequest} or
	 * a {@link ChunkKo} message that:
	 * <ol>
	 * <li>checks whether the requested chunk is locally available or not</li>
	 * <li>if the chunk was available, reply with a {@link ChunkMessage}
	 * message;<br>
	 * if the chunk was not available, reply with a {@link ChunkMissing} message
	 * </li>
	 * </ol>
	 * <b>Note:</b> the {@link ChunkMessage} has to travel over the unreliable
	 * transport while the {@link ChunkMissing} has to travel over the reliable
	 ...
	 * 
	 * @param req
	 *            The request for a chunk (retransmission)
	 */
	private void tryAndSendChunk(ChunkRequest req) {
		log("[RCV] " + req);
		Chunk<?> chunk = store.getChunk(req.getSessionId(), req.getChunkId());
...
>>>>>>> Latency, Packet Loss, Chunk TTL Fix
